...
### Smooth Manifold
As a set "Minskowski spacetime" is $$\mathbb{R}^4$$. This set is then equipped with the [Euclidean topology](https://en.wikipedia.org/wiki/Euclidean_topology) by: (1) equipping $$\mathbb{R}^4$$ with the [Euclidean norm](https://en.wikipedia.org/wiki/Euclidean_space#Euclidean_norm), (2) defining a metric on $$\mathbb{R}^4$$ using this Euclidean norm, and (3) defining the Euclidean topology as the topology generated by the open balls of this metric. The set $$\mathbb{R}^4$$ equipped with this Euclidean topology makes $$\mathbb{R}^4$$ a topological manifold.
...
governs when a Lorentz, and thus Minkowski exists.

So in order to introduce a Minkowski metric on $$\mathbb{R}^4$$ we must first introduce a smooth, global vector field on $$\mathbb{R}^4$$. Traditionally this smooth, global vector field is taken to be $$t^\mu \equiv (1,0,0,0)$$ in the standard global coordinates on $$\mathbb{R}^4$$. This yields the standard Minkowski metric on $$\mathbb{R}^4$$.
