...
For example, to define the specturm of a self-adjoint element $$a$$ in an algebra $$\mathfrak{U}(\mathbf{B})$$ that doesn't have a unit, one has to, through formal adjunction, add a unit to $$\mathfrak{U}(\mathbf{B})$$, then compute the specturm of $$a$$ in this larger algebra. So the possible experimental values $$\sigma(a)$$ will appear as if $$\mathfrak{U}(\mathbf{B})$$ has a unit.

As the possible experimental values when a unit is not present are identical to when a unit is present, we will simply assume that all such $$\mathfrak{U}(\mathbf{B})$$ contain a unit.

An even stronger argument for always including a unit is the observation that the possible experimental values $$\sigma(a)$$ are simply undefined if one dogmatically insists on not including a unit in $$\mathfrak{U}(\mathbf{B})$$, even if it is only for the purposes of defining a specturm.

So if we adopt the position that a unit is always present, one convenient way to include a unit, and the one we adopt, is to assume that the empty set $$\emptyset$$ is assigned the C\*-algebra $$\mathbb{C} \mathbf{1}$$ generated by the unit operator $$\mathbf{1}$$

$$
\emptyset \rightarrow \mathfrak{U}(\emptyset) \equiv \mathbb{C} \mathbf{1}.
$$

As the empty set $$\emptyset$$ is a member of any $$\mathbf{B}$$, isotony implies $$\mathfrak{U}(\emptyset) \subset \mathfrak{U}(\mathbf{B})$$. Thus $$\mathfrak{U}(\emptyset) \equiv \mathbb{C} \mathbf{1}$$ implies that all $$\mathfrak{U}(\mathbf{B})$$ contain a unit.
