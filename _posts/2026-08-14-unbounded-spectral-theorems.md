...
Finally, $$\sigma(A)$$ is a compact metric measurable space, by [**Lemma** *(The Spectrum is a Compact Metric Measurable Space)*](../spectral-theorems/#lmm:spectrum-is-compact-metric-measurable). So [**Lemma** *(hall-prblm-8.3.3c)*](../spectral-theorems/#lmm:hall-prblm-8.3.3c) applies and gives that $$\mathcal{F}$$ consists of *all* bounded measurable complex-valued functions on $$\sigma(A)$$, which is **Part 1**.
...
**($$\subset$$).** Let $$\mu \in \sigma\big(p(A,A^*)\big)$$. Fix an integer $$n \ge 1$$ and put $$\varepsilon_n \equiv 1/n > 0$$. By [**Lemma** *(hall-10.27)*](#lmm:hall-10.27) there is a nonzero closed subspace $$W^{\varepsilon_n}$$, invariant under $$A$$ and $$A^*$$, every nonzero element of which is an $$\varepsilon_n$$-almost eigenvector for $$p(A,A^*)$$ with eigenvalue $$\mu$$.
